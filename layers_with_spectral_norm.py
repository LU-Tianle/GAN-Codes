...
class Dense:
    def __init__(self, units, kernel_initializer, use_bias=False, bias_initializer=None, activation=None, spectral_norm=True, name=None):
        """
    ...
    def __call__(self, batch, training, name):
        ...
        if self.spectral_norm:
            weights = spectral_normalization(weights, iteration=1, name=name)
        with tf.name_scope(name):
            batch = tf.matmul(batch, weights, name='matmul')
            if self.use_bias:
                batch = batch + bias
            if self.activation is not None:
                batch = self.activation(batch, name='activation')
        return batch
...
class Conv2D:
    def __init__(self, filters, kernel_size, kernel_initializer,
                 ...
                 spectral_norm=True, data_format="channels_first", name=None):
        """
    ...
    def __call__(self, batch, training, name):
        ...
        if self.spectral_norm:
            self.kernel = spectral_normalization(kernel, iteration=1, name=name)
        with tf.name_scope(name):
            conv2d = tf.nn.conv2d(batch, filter=kernel, strides=strides, padding=self.padding, data_format=self.data_format, name='conv')
            if self.use_bias:
                conv2d = tf.nn.bias_add(conv2d, bias, data_format=self.data_format, name='add_bias')
            if self.activation is not None:
                conv2d = self.activation(conv2d, name='activation')
        return conv2d
