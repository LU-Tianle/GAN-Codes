...
class GeneratorDcgan:
    ...
    def __init__(self, image_shape, first_conv_trans_layer_filters, conv_trans_layers):
        """
        ...
        for layer in range(conv_trans_layers - 1):
            ...
            self.conv_trans_batch_norm_layers.append((conv_trans, batch_norm))
    ...
    def __call__(self, batch_z, training):
        ...
        for (conv_trans, batch_norm) in self.conv_trans_batch_norm_layers:
...
class DiscriminatorDcgan:
    def __init__(self, first_layer_filters, conv_layers):
        ...
        for layer in range(2, conv_layers + 1):
            ...
            self.conv_batch_norm_layers.append((conv, batch_norm))
    ...
    def __call__(self, batch, training):
        ...
        for (conv, batch_norm) in self.conv_batch_norm_layers:
            batch = conv(batch)
            batch = batch_norm(batch, training=training)
            batch = tf.nn.swish(batch)
